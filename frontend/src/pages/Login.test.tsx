...
describe('Login page', () => {
  ...
  it('logs in and navigates to root on success', async () => {
    ...
    fireEvent.change(screen.getByLabelText('이메일 또는 전화번호'), { target: { value: 'user@yoga.com' } });
    fireEvent.change(screen.getByLabelText('비밀번호'), { target: { value: 'password' } });
    fireEvent.click(screen.getByRole('button', { name: '로그인' }));

    await waitFor(() => expect(loginMock).toHaveBeenCalledWith('user@yoga.com', 'password'));
  ...
  it('shows parsed error message on login failure', async () => {
    ...
    fireEvent.change(screen.getByLabelText('이메일 또는 전화번호'), { target: { value: '01012341234' } });
